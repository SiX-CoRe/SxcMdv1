...
let handler = async (m, { Lumnztyz6x, text, isCreator, reply }) => {
  if (!isCreator) return reply("⚠️ Fitur ini hanya bisa digunakan oleh Developer bot!");

  const channels = await Lumnztyz6x.newsletterFetchAllParticipating();
  ...
  if (m.chat.endsWith("@newsletter")) {
    ...
    try {
      if (action === "on") {
        await Lumnztyz6x.newsletterMute(m.chat);
        reply(`Notifikasi Channel ini berhasil dimatikan.`);
      } else {
        await Lumnztyz6x.newsletterUnmute(m.chat);
  ...
  for (const idx of indexes) {
    ...
    try {
      if (isOn) {
        await Lumnztyz6x.newsletterMute(target.id);
        hasil.push(`🔕 ${target.name || target.id} dimute.`);
      } else if (isOff) {
        await Lumnztyz6x.newsletterUnmute(target.id);
