...
let handler = async (m, { Lumnztyz6x, qmsg, mime, isCreator, text, reply }) => {
  ...
  if (m.quoted && /image/.test(mime)) {
    try {
      const mediaPath = await Lumnztyz6x.downloadAndSaveMediaMessage(qmsg);
  ...
  try {
    const newsletter = await Lumnztyz6x.newsletterCreate(name.trim(), desc, { url: imageUrl });
